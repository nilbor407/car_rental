...
route.post("/login_now", async function (req, res) {
  ...
  if (data.length > 0) {
    // Generate JWT token
    const token = jwt.sign({ username: data[0].username, user_id: data[0].user_id }, 'secret_key', { expiresIn: '1h' });
...
route.get("/home",verifyToken, async function(req,res){
